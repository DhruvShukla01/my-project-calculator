...
# author: Dhruv Shukla
# date: May 15, 2023
# file: calculatorGUI.py a Python program that implements an interactive calculator for the user
# input: user input equations.
# output: interactive implementation with clickable buttons for a GUI for user.
from tkinter import *
...
def clickButton(entrybox, value):
    eq = entrybox.get()
    # if value entered is c, clears the screen
    if value == 'c':
        entrybox.delete(0, END)
    # if value == '=', evaluates the equation (eq) and then clears the screen
    elif value == '=':
        result = calculate(eq)
        entrybox.delete(0, END)
        entrybox.insert(END, str(result))
    # inserts the added values by the user
    else:
